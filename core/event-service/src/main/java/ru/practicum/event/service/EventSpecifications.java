...
public class EventSpecifications {
    ...
    // ИСПРАВЛЕНО: работаем с initiatorId, а не initiator
    public static Specification<Event> withUsers(List<Long> users) {
        if (users == null || users.isEmpty())
            return null;
        return (root, query, cb) ->
                root.get("initiatorId").in(users); // ← ВАЖНО: initiatorId
    }

    // ИСПРАВЛЕНО: работаем с categoryId, а не category.id
    public static Specification<Event> withCategoriesIn(List<Long> categories) {
        if (categories == null || categories.isEmpty())
            return null;
        return (root, query, cb) ->
                root.get("categoryId").in(categories); // ← ВАЖНО: categoryId
    }
    ...
    public static Specification<Event> withOnlyAvailable(Boolean onlyAvailable) {
        ...
        return (root, query, cb) -> {
            // Используем confirmedRequests как transient поле
            return cb.or(
                    cb.equal(root.get("participantLimit"), 0),
                    cb.lessThan(
                            cb.coalesce(root.get("confirmedRequests"), 0),
                            root.get("participantLimit")
                    )
            );
        };
